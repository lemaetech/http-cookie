...
let parse_cookie_av attr_value err =
  let rec validate i av =
    if i >= String.length av then
      av
    else
      let c = av.[i] in
      if is_control_char c || Char.equal c ';' then
        err c
      else
        validate (i + 1) av
  in
...
let parse_value value =
  ...
  let rec validate i s =
    if i >= String.length s then
      ...
      then
        ...
        err "Invalid char '%c' found in cookie value" c
  in
  let strip_quotes s =
  ...
  let value =
    if String.length value > 0 then
      ...
      err "Cookie value length must be > 0."
  in
...
let to_set_cookie_header_value t =
  ...
  O.iter
    (fun same_site ->
      if Same_site.(equal Default same_site) then
        add_str "; SameSite"
      else
        add_str "; SameSite=%s" (Same_site.to_string same_site))
    t.same_site;
